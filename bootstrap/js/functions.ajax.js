...
//caso contrario manejamos mensajes específicos
                        else{
                            $('#alertBoxes').html('<div class="box-error"></div>');
                            if (typeof msj === 'string' && msj.indexOf('SQLErr:') === 0){
                                $('.box-error').hide(0).html('Error en la base de datos: ' + msj.replace('SQLErr:',''));
                            } else if (typeof msj === 'string' && msj.indexOf('DBERR:') === 0){
                                $('.box-error').hide(0).html('Error al conectar con la base de datos.');
                            } else {
...
//si la sesion se inicia correctamente presentamos el mensaje
                    success:function(msj){
                        ...
                        $('#alertBoxes').html('<div class="box-error"></div>');
                        if (msj === 'EXISTS'){
                            $('.box-error').hide(0).html('El usuario ya existe, por favor elija otro nombre.');
                        } else if (typeof msj === 'string' && msj.indexOf('SQLErr:') === 0){
                            $('.box-error').hide(0).html('Error en la base de datos: ' + msj.replace('SQLErr:',''));
                        } else if (typeof msj === 'string' && msj.indexOf('DBERR:') === 0){
                            $('.box-error').hide(0).html('Error al conectar con la base de datos.');
                        } else {
