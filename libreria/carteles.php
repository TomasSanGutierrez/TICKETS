...
function traer_datos($nro=0) // declara el constructor, si trae el numero de persona lo busca 
 ...
 static function categorias(){
    // Exclude help category from default list so it only appears when explicitly requested
    $sql="select categoria,count(id_cartel) from carteles where activo=1 AND categoria <> 'Ayuda' group by categoria";
